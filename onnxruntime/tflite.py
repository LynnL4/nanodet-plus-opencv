...
class my_nanodet():
    def __init__(self, model_pb_path, label_path, prob_threshold=0.4, iou_threshold=0.3):
        ...
        self.net = tf.lite.Interpreter(model_path=model_pb_path)
        self.net.allocate_tensors()
        self.input_details = self.net.get_input_details()
        self.output_details = self.net.get_output_details()
        self.input_shape = (self.input_details[0]['shape'][1], self.input_details[0]['shape'][2])
        self.reg_max = int((self.output_details[0]['shape'][2] - self.num_classes) / 4) - 1
    ...
    def detect(self, srcimg):
        ...
        self.net.set_tensor(self.input_details[0]['index'], blob)
        self.net.invoke()
        outs = self.net.get_tensor(self.output_details[0]['index']).squeeze(axis=0)
